...
export const TakeActionScreen: React.FC<Props> = ({ selectedSteps, onNavigate }) => {
  ...
  const selectedRedirectObj = redirects.find(r => r.id === selectedRedirect);
  ...
  return (
    <div className="ta">
      ...
      {activeTab === "subscriptions" && (
        <>
          {/* ── Section 1: Suggested Next Steps recap ── */}
          <div className="card ta__steps-recap">
            <div className="section-title ta__steps-recap-title">SUGGESTED NEXT STEPS</div>
            {selectedSteps.map(step => (
              <div key={step.id} className="ta__recap-row">
                <span className="ta__recap-text">{step.text}</span>
                <div className={`ta__recap-check${step.category === activeTab ? " ta__recap-check--checked" : ""}`}>
          ...
          <div className="ta__agent">
            ...
            <div className="ta__agent-text">
              ...
              <p><strong>I recommend you keep {recommended?.name ?? "Asana"} as your main tool and pause the others.</strong></p>
            </div>
          </div>

          <div className="card ta__ranking-card">
            <div className="section-title ta__ranking-title">SUBSCRIPTION PRIORITY RANKING</div>
            {subscriptions.map(sub => (
              <div
                key={sub.id}
                className={`ta__ranking-row${sub.recommended ? " ta__ranking-row--keep" : " ta__ranking-row--pause"}`}
              >
                {sub.name}
              </div>
            ))}
          ...
          <div className="card ta__redirects-card">
            <div className="section-title ta__redirects-title">SUGGESTED CASH REDIRECTS</div>
            {redirects.map(r => (
              <div
                key={r.id}
              ...
              >
                ...
                <div className={`ta__redirect-check${selectedRedirect === r.id ? " ta__redirect-check--checked" : ""}`}>
                  {selectedRedirect === r.id && <CheckIcon />}
                </div>
              </div>
            ))}
          </div>
          ...
          {selectedRedirect && (
            <>
              <div className="ta__agent">
                <PulseAvatar />
                <div className="ta__agent-text">
                  <p>I see you've chosen to "{selectedRedirectObj?.text}"</p>
              ...
              <div className="card ta__impact-card">
                <div className="section-title ta__impact-title">HOW WOULD THIS IMPACT CASH FLOW?</div>
                {impactLoading ? (
                  <div className="ta__impact-loading">Loading projection…</div>
                ) : impact ? (
                  <>
                    <div className="ta__charts">
                      <div className="ta__chart-col">
                        ...
                        <p className="ta__chart-label">Current projected lowest cash balance</p>
                      </div>
                      <div className="ta__chart-col">
                        <div className="ta__chart-wrap">
                          <CashFlowChart
                        ...
                        <p className="ta__chart-label">New projected lowest cash balance</p>
                      </div>
                    </div>
              ...
              <p className="ta__finish-hint">
                <strong>Please review and click 'Finish' if this issue is done and I'll close this case.</strong>
              </p>
            </>
          )}
        </>
      ...
      {activeTab === "payroll" && (
        <div className="ta__agent">
          <PulseAvatar />
          <div className="ta__agent-text">
            <p>I see you've chosen to "Slow down hiring, trim hours where possible, and make sure future payroll increases are tied to revenue growth."</p>
      ...
      <div className="ta__action-row">
        ...
        {selectedRedirect ? (
          <button className="btn btn--green ta__finish-btn" onClick={handleFinish}>
            Finish
          </button>
        ) : (
          <button className="btn btn--ghost ta__next-btn" onClick={() => onNavigate("cashRedirects")}>
            Next
          </button>
        )}
      </div>

      <p className="ta__disclaimer">*Pulse is AI and can make mistakes. Important information should always be checked before proceeding.</p>
    </div>
  );
};
